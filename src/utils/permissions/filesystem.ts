...
export function getClaudeSkillScope(
  ...
  const bases = [
    ...
    {
      dir: expandPath(join(homedir(), '.freecc', 'skills')),
      prefix: '~/.freecc/skills/',
    },
  ]
 ...
 * Path format: ~/.freecc/projects/{sanitized-cwd}/...
 */
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  if (claudeFolderAllowRule) {
    ...
    // Accepts both the broad patterns ('/.claude/**', '~/.freecc/**') and
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.freecc/jobs/. Symlink guard: every
  ...
  // ~/.freecc/, which is in DANGEROUS_DIRECTORIES. The CLAUDE_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .claude/ only (not ~/.freecc/) since launch.json is per-project.
  if (
...
export function checkReadableInternalPath(
  ...
  // Tasks directory (~/.freecc/tasks/) for swarm task coordination
  ...
  // Teams directory (~/.freecc/teams/) for swarm coordination
