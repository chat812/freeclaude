 ...
 * user's ~/.freecc/settings.json can't redirect requests away from the
 ...
 *
 * - userSettings (~/.freecc/settings.json): controlled by the user, not project-specific
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.freecc.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.freecc/settings.json env (gh#217). policy/flag
