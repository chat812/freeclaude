 ...
 * to the user's ~/.freecc/settings.json API-key config (apiKeyHelper,
 ...
 * from ~/.freecc/settings.json or project settings is ignored.
 */
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running awsAuthRefresh (in settings or ~/.freecc.json):',
            )
  ...
  try {
    ...
    try {
      ...
      const message = chalk.red(
        'Error getting AWS credentials from awsCredentialExport (in settings or ~/.freecc.json):',
      )
