 ...
 * user-controlled files (~/.freecc/settings.json and ~/.freecc.json),
 ...
 *
 * We read from global config (~/.freecc.json) and user settings
 * (~/.freecc/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.freecc/settings.json),
